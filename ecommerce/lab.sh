...
#                     sdd 문서·테스트(스펙)·빌드 설정·게이트는 그대로 둔다.
#   ./lab.sh solve    정답 구현(solution/)을 src/main 으로 복원한다. 라이브가
#                     막혔을 때의 폴백, 또는 완성본 확인용.
#   ./lab.sh verify   DDD 경계 게이트 + gradle 단위·E2E 테스트를 돌린다.
#   ./lab.sh status   현재 상태(구현 존재 여부)를 보여준다.
#
# 어느 명령이든 여러 번 실행해도 같은 결과가 된다. reset→build→verify→reset 을
# 반복해도 매번 동일하게 23/23 로 수렴한다.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"
IMPL="src/main/java/kr/elice/shop"
SOL="solution/java/kr/elice/shop"
...
reset() {
  ...
  echo "        남아 있는 것: sdd/ 문서, src/test/ (스펙), build.gradle, 게이트."
  status
}

solve() {
  if [ ! -d "$SOL" ]; then
    echo "[solve] solution/ 스냅샷이 없습니다." >&2; exit 1
  fi
  rm -rf "src/main/java/kr"
  mkdir -p "$(dirname "$IMPL")"
  cp -R "$SOL" "$(dirname "$IMPL")/"
